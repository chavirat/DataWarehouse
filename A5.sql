-- Assignment 5: using Subtotal Operators and Analytic Functions
...
--2.
SELECT Name, CalYear, CalQuarter,  SUM(ExtCost) AS TotalExtCost, COUNT(*) AS NumberInventoryTrans
FROM inventory_fact, date_dim, cust_vendor_dim
WHERE inventory_fact.DateKey = date_dim.DateKey
AND inventory_fact.CustVendorKey = cust_vendor_dim.CustVendorKey
...
--3.
SELECT SecondItemId,  SUM(ExtCost) AS TotalExtCost, SUM(Quantity) AS TotalQuantity, COUNT(*) AS NumberInventoryTrans
...
UNION
SELECT SecondItemId,' ', SUM(ExtCost) AS TotalExtCost, SUM(Quantity) AS TotalQuantity
FROM inventory_fact, item_master_dim, branch_plant_dim
WHERE inventory_fact.ItemMasterKey = item_master_dim.ItemMasterKey
AND inventory_fact.BranchPlantKey = branch_plant_dim.BranchPlantKey
AND TransTypeKey = 2
...
UNION
SELECT ' ',' ', SUM(ExtCost) AS TotalExtCost, SUM(Quantity) AS TotalQuantity
FROM inventory_fact, item_master_dim, branch_plant_dim
WHERE inventory_fact.ItemMasterKey = item_master_dim.ItemMasterKey
AND inventory_fact.BranchPlantKey = branch_plant_dim.BranchPlantKey
AND TransTypeKey = 2;
...
RANK() OVER (ORDER BY SUM(ExtCost) ) AS ExtCostRank
...
ORDER BY SUM(ExtCost)) AS ExtCostRank
FROM inventory_fact, cust_vendor_dim
WHERE inventory_fact.CustVendorKey = cust_vendor_dim.CustVendorKey
AND TransTypeKey = 5
GROUP BY state, Name
...
--3.
SELECT Name, COUNT(*) AS NumberInventoryTrans,
RANK() OVER (ORDER BY SUM(TransTypeKey) ) AS Rank_InventoryTrans,
DENSE_RANK() OVER (ORDER BY SUM(TransTypeKey) ) AS DenseRank_InventoryTrans
FROM inventory_fact, cust_vendor_dim
WHERE inventory_fact.CustVendorKey = cust_vendor_dim.CustVendorKey
AND TransTypeKey = 5
GROUP BY Name;
...
ORDER BY CalYear DESC, TotalExtCost DESC;
